...
| `aim-kind` | `AimOntologyKind` | Entity archetype: `'uc'` \| `'act'` \| `'cls'` \| `'obj'` \| `'per'` \| `'plc'` \| `'rol'`. |
...
| `aim-href` | `string?` | Target URI for navigation and deep linking; renders Portuguese Bicolor Portal Door `<path class="aim-portal-door">` and `<a href="..." target="_blank">` in standalone vector SVG. |

#### The 7 Canonical `AimOntologyKind` Archetypes
...
5. **`per` (Person / Actor):** Living human or autonomous role filler with head circle and torso arc (Gold initiating, Graphite defined).
6. **`plc` (Place / Where):** Spatial venue or architectural stage card with blue accent header (`#3B82F6`).
7. **`rol` (Role):** Structural KL-ONE constraint card with dashed graphite border (`strokeDasharray: '4,3'`).
...
| `onNodeClick` | `(node: RaidNodeData, event: MouseEvent) => void` | — | Fired on left-hemisphere (Persona / Anchor) taps; consumed by AIA to toggle the In Situ Inspector Drawer. |
| `onNodePortalClick` | `(node: RaidNodeData, event: MouseEvent) => void` | — | Fired on right-hemisphere (Portuguese Bicolor Portal Door) taps; consumed by AIA for route transitions. |
| `onNodeDblClick` | `(node: RaidNodeData, event: MouseEvent) => void` | — | Deprecated legacy desktop double-click fallback. |
...
#### The Duality of the Object & Ontological Navigation (CR033)
* **The Master Treatise:** Formalized in [`doc/DualityOfTheObject.md`](file:///Users/RSB/Projects/GitHub/RaidCanvas/doc/DualityOfTheObject.md).
* **The Portuguese Bicolor Seam:** When a node carries `aim-href`, its right half ($x \ge \text{midX}$) renders a translucent Cascais Heraldic Green portal wash (`fill="rgba(16, 185, 129, 0.10)"`, class `.aim-portal-door`) and quiet doorway chevron (`<text class="aim-portal-chevron">›</text>`).
* **Selective Revelation:** Unlinked nodes omit the portal door and chevron entirely, remaining monolithic archetypes.
* **Touch-First iPad Ergonomics:** Replaces desktop double-clicks with single-tap hemisphere disambiguation:
  - **Left Tap (`onNodeClick`):** Opens the DaisyUI Inspector Drawer beside the canvas without unmounting or losing canvas state.
  - **Right Tap (`onNodePortalClick`):** Directly steps through the door into the deeper entity / sub-model.
* **Fitts' Law Mastery:** In a standard 160×70px node, the portal door provides an 80×70px thumb target for iPad slates.
* **Drag-Immunity Navigation Guard:** Pointer drag movement ($\Delta > 4\text{px}$ or moving events) suppresses both click callbacks, preventing accidental navigation while moving shapes.
* **Vector SVG Hyperlinks:** Exported vector SVGs wrap the right hemisphere portal door in standard `<a href="..." target="_blank">`, maintaining functional vector links in Chrome, Safari, Keynote, and PDF viewers.
